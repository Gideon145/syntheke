"""Create the FLAGSHIP mainnet treaty on V3 — DEX-subject pact with LIVE OKX
market-data conditions (bits 11/12), paid via x402 by a fresh payer wallet."""
...
body = json.dumps({
    "partyADesc": "Liquidity guardian agent",
    "partyBDesc": "DEX operations client",
    "description": "DEX liquidity guardian: Provider A maintains Client B's ETH-USDT liquidity depth on the OKX DEX within 2 percent of target and alerts within 60 seconds of any deviation, verified against live OKX market data every 15 seconds",
}).encode()
...
try:
    with urllib.request.urlopen(req, timeout=480) as r:
        ...
        print(json.dumps({"payer": payer.address,
                          **{k: d.get(k) for k in ("result", "pactId", "state", "subject", "treasuryFee", "error")}}, indent=1))
